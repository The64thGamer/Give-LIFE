...
const MAX_CHASE_SPEED: float = 4.0  # full cycles per second at chase = 1.0

var visible_energy: float = 0.0
var current_chase: float = 0.0       # 0.0 = stopped, 1.0 = max speed
...
func _sent_signals(signal_ID: String, the_signal):
	...
	match signal_ID:
		"intensity":
			...
			_update_light()
		"chase":
			current_chase = clamp(the_signal, 0.0, 1.0)
		"grouping":
...
func _update_light():
	if total <= 0:
		return
	var shifted_index = (number + int(round(_chase_offset * total))) % total
